...
const BattleScreen = ({ section }: BattleScreenProps) => {
  const [profiles, setProfiles] = useState<ProfileData[]>([]);
  const [currentPair, setCurrentPair] = useState<[ProfileData | null, ProfileData | null]>([null, null]);
  ...
  const showNextPair = () => {
    const currentIdx = profiles.findIndex(p => p.name === currentPair[1]?.name);
  ...
  const ProfileCard = ({ profile, onVote }: { profile: ProfileData; onVote: () => void }) => (
    <motion.div
      initial={{ opacity: 0, scale: 0.8 }}
      animate={{ opacity: 1, scale: 1 }}
      exit={{ opacity: 0, scale: 0.8 }}
      transition={{ type: "spring", stiffness: 300, damping: 30 }}
      className="bg-white rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-6 md:p-8 flex flex-col items-center relative cursor-pointer hover:shadow-xl transition-shadow max-w-lg mx-auto w-full"
      onClick={onVote}
    >
      {/* Profile Image */}
      <motion.div 
        className="w-28 h-28 sm:w-36 sm:h-36 md:w-44 md:h-44 rounded-full overflow-hidden mb-4 sm:mb-6 border-4 border-white shadow-lg"
      ...
      <div className="text-center w-full">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-2 sm:mb-3">{profile.name}</h2>
        <p className="text-gray-600 text-sm sm:text-base md:text-lg mb-4 sm:mb-6">
          {profile.name} is a {profile.age} year old student from {profile.location}.
        </p>

        {/* Bullet Points */}
        <div className="space-y-2 sm:space-y-3 text-left px-2 sm:px-4">
          {[profile.bio, ...profile.interests].map((point, idx) => (
            <motion.div 
              key={idx} 
            ...
            >
              <span className="text-gray-400 mt-1 sm:mt-1.5 text-base sm:text-lg">○</span>
        ...
        <div className="mt-4 sm:mt-6 md:mt-8">
          <a 
            ...
            className="inline-block text-gray-600 hover:text-gray-800 transition-colors scale-110 sm:scale-125"
  ...
  return (
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      {/* Main Content */}
      <main className="flex-1 w-full max-w-[90rem] mx-auto px-4 sm:px-6 md:px-8 flex flex-col">
        {/* Title */}
        <div className="text-center mb-4 pt-20">
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mb-2">Who's Your Pick?</h1>
          <p className="text-sm sm:text-base text-gray-500 px-4">Click on your favorite profile to vote</p>
        </div>

        {/* Profile Cards Container */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 lg:gap-32 max-w-[80rem] mx-auto relative">
          ...
          {/* VS Badge */}
          <div className="flex md:absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 justify-center my-3 md:my-0">
            <div className="bg-rose-500 text-white px-6 sm:px-8 md:px-10 py-3 sm:py-4 md:py-5 rounded-full font-bold text-base sm:text-lg md:text-xl shadow-xl">
              VS
            </div>
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="text-center py-2 sm:py-3 text-gray-400 text-xs sm:text-sm">
